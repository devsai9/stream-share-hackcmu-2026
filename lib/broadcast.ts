...
export type TrackRenamedPayload = {
  senderId: string
  track: Track
}
...
export async function joinProjectChannel(
  projectId: string,
  handlers: {
    onNotesUpdated?: (payload: unknown) => void
    onCursorMoved?: (payload: unknown) => void
  ...
  channel
    ...
    .on('broadcast', { event: 'notes-updated' }, (payload) => {
      handlers.onNotesUpdated?.(payload)
    })
    .on('broadcast', { event: 'cursor-moved' }, (payload) => {
      handlers.onCursorMoved?.(payload)
    })
    ...
    .on('broadcast', { event: 'track_renamed' }, (payload) => {
      handlers.onTrackRenamed?.(payload)
    })
...
export function broadcastTrackRenamed(
  channel: RealtimeChannel,
  senderId: string,
  track: Track,
) {
  return broadcast(channel, 'track_renamed', { senderId, track } satisfies TrackRenamedPayload)
}
